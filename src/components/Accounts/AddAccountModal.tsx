...
export function AddAccountModal({ onClose, onSuccess }: AddAccountModalProps) {
  ...
  const [formData, setFormData] = useState({
    name: '',
    bank_name: '',
    type: 'debit' as 'debit' | 'credit' | 'savings' | 'investment',
    balance: '',
    credit_limit: '',
    cut_off_day: '',
    payment_due_day: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.type === 'credit') {
      insertData.cut_off_day = formData.cut_off_day ? parseInt(formData.cut_off_day) : null;
      insertData.payment_due_day = formData.payment_due_day ? parseInt(formData.payment_due_day) : null;
      insertData.billing_period_start_day = formData.billing_period_start_day ? parseInt(formData.billing_period_start_day) : null;
      insertData.billing_period_end_day = formData.billing_period_end_day ? parseInt(formData.billing_period_end_day) : null;
    }

    const { error: dbError } = await supabase.from('accounts').insert(insertData);
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[60] overflow-y-auto">
      <div className="bg-[#1a3a2e] rounded-2xl max-w-md w-full p-4 sm:p-6 my-8 border border-emerald-500/30 max-h-[calc(100vh-4rem)]  overflow-y-auto">
        <div className="flex justify-between items-center mb-4 sm:mb-6">
          <h3 className="text-lg sm:text-xl font-bold text-white">Nueva Cuenta</h3>
          <button
            onClick={onClose}
          ...
          {formData.type === 'credit' && (
            <>
              ...
              <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-4">
                ...
                <div className="px-4 py-3 bg-gray-900/50 border border-gray-700 rounded-lg text-gray-400 text-lg font-mono">
                  $ 0.00
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  ℹ️ Este campo se calculará automáticamente sumando los gastos del periodo configurado
                </p>
              </div>
