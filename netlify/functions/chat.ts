...
// ── Execute Function Called by Gemini ────────────────────────
...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    if (!parsed.message || !parsed.accessToken) {
        return {
            statusCode: 400,
            body: JSON.stringify({ error: 'Missing message or accessToken' }),
        };
    }
    ...
    try {
        ...
        return {
            statusCode: 200,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ reply, action: actionResult }),
        };
    } catch (err: any) {
